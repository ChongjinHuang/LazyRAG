...
import { ArrowRightOutlined, FolderOpenOutlined, GoogleOutlined } from "@ant-design/icons";
...
import type { FeishuDataSourceConnection } from "../../oauth/types";
...
export default function DataSourceProviderPanel({ vm }: { vm: DataSourceManagementVm }) {
  ...
  useEffect(() => {
    let active = true;
    void dataSourceCloudOauthApi
      .listConnectionsApiAuthserviceV1CloudConnectionsGet({
        provider: "googledrive",
        status: null,
      })
      .then((response) => {
        if (!active) return;
        const connection = getCloudConnectionItems(response.data)
          .map((item) => mapCloudConnectionToDataSourceConnection(item, "googledrive"))
          .find((item) => item.status === "connected" && Boolean(item.connectionId));
        setGoogleDriveConnection(connection || null);
      })
      .catch(() => {
        if (active) setGoogleDriveConnection(null);
      });
    return () => {
      active = false;
    };
        ...
        >
          ...
          <span className="data-source-provider-card-copy">
            <span className="data-source-provider-title-row">
              ...
              <Tag color={googleDriveConnection ? "success" : "default"}>
            ...
            <span className="data-source-provider-desc">
              {googleDriveConnection
                ? t("admin.dataSourceGoogleDriveConnected", {
                    account: googleDriveConnection.accountName,
                  })
